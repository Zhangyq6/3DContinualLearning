...
from backbone import build_model_from_cfg
# utils
...
def save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, prefix, args, logger = None):
    if args.local_rank == 0:
        torch.save({
                    ...
                    'epoch' : epoch,
                    'metrics' : metrics.state_dict() if metrics is not None and hasattr(metrics, 'state_dict') else metrics,
                    'best_metrics' : best_metrics.state_dict() if best_metrics is not None and hasattr(best_metrics, 'state_dict') else best_metrics,
