...
class EaseCosineLinear(nn.Module):
    ...
    def forward_reweight(self, input, cur_task, alpha=0.1, beta=0.0, init_cls=10, last_cls=-1, inc=10, out_dim=768, use_init_ptm=False):
        for i in range(cur_task + 1):
            if i == 0:
                start_cls = 0
                end_cls = init_cls
            elif i == cur_task and last_cls != -1:
                start_cls = init_cls + (i - 1) * inc
                end_cls = start_cls + last_cls
            else:
                start_cls = init_cls + (i - 1) * inc
