...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, pos, global_feature=None, token_position=None, level1_center=None, level1_index=None, level2_center=None, level2_index=None, batch_idx=None):
        for idx, block in enumerate(self.blocks):
            if idx < self.config.prompt_depth and token_position is not None: #6
...
class PointTransformer_pointtokenprompt(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.prompt_init = config.prompt_key_init
        self.embedding_key = config.embedding_key
        self.point_number = config.point_number
        self.prompt_key = config.prompt_key
        self.pool_size = config.size
        self.top_k = config.top_k
        self.batchwise_prompt = config.batchwise_prompt
        self.prompt_key_init = config.prompt_key_init
        ...
        if self.point_number is not None and self.pool_size is not None and self.prompt_pool: 
            self.prompt = Prompt(point_number=self.point_number, embed_dim=self.trans_dim, embedding_key=self.embedding_key, prompt_init=self.prompt_init,
                    prompt_pool=self.prompt_pool, prompt_key=self.prompt_key, pool_size=self.pool_size, top_k=self.top_k, batchwise_prompt=self.batchwise_prompt,
    ...
    def forward(self, pts, task_id=-1, cls_features=None, train=False, batch_idx=None):
        visualize = False
        ...
        if self.prompt_pool:
            prompt_mask = torch.full((pts.shape[0], 1), task_id)
            out = self.prompt(pts, prompt_mask = prompt_mask, cls_features = cls_features)
            pts = out['prompted_embedding']
