...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, args, _trsf = True, join_thelast = False):
        ...
        self._setup_data(dataset_name, shuffle, seed, args, trsf= _trsf)
    ...
    def get_olddata(self, per_class_num):
        ...
        for idx in range(0, self.nb_tasks):
            ...
            for id in range(start_idx, start_idx + self._increments[idx]):
                ...
                val_indx = np.random.choice(len(class_data), min(len(class_data), per_class_num), replace=False)
    ...
    def get_dataset(
        self, indices, source, mode, appendent=None, ret_data=False, m_rate=None, old_data = None, cur_task = 0, size = 0
    ):
        if source == "train":
            x, y = self._train_data, self._train_targets
        elif source == "test":
        ...
        for idx in indices:
            ...
            if size !=0:
                np.random.seed(42)
                index = np.random.choice(class_targets.shape[0],size = min(20,class_targets.shape[0]), replace = False)
                class_data = class_data[index]
                class_targets = class_targets[index]
    ...
    def _setup_data(self, dataset_name, shuffle, seed, args, trsf = True):
        ...
        # Order [3, 13, 1, 12, 7, 5, 6, 8, 14, 2, 0, 9, 11, 10, 4]
        ...
        print("Class Order is: ",self._class_order)
...
def _get_idata(dataset_name, args=None):
    ...
    if name in "modelnet40":
        return ModelNet()
    elif name in "shapenet":
        return ShapeNet()
    elif name in "scanobject":
        return ScanObject()
