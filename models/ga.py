...
class Learner(BaseLearner):
    def __init__(self, args, model_config):
        ...
        self._network = BaseNet(args, True, modelconfig = model_config)
    ...
    def after_task(self):
        ...
        if not self.args["debug"]:
            self.save_checkpoint(os.path.join(self.args["ckpt_path"],"{}_{}_{}".format(self.args["dataset"],self.args["seed"],self.args["use_old_data"])))
    ...
    def incremental_train(self, data_manager):
        self._cur_task += 1
        self._total_classes = self._known_classes + data_manager.get_task_size(self._cur_task) #data_manager._increments = [3, 3, 3, 3, 3]
        self._network.last_cls = data_manager.get_task_size(-1)
        self._network.nb_tasks = data_manager.nb_tasks
        # self._network.update_fc(self._total_classes, inc = data_manager.get_task_size(self._cur_task))
        ...
        self.old_data = data_manager.get_olddata(per_class_num=self.samples_per_class)
        ...
        print("Train Data Number is : ",self.all_data_num)
        print("Using Old Data Number is : ", self.old_data_num)
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            info = "Task {}, Epoch {}/{} => L {:.3f}, Lce {:.3f},T_acc {:.2f}".format(
                    self._cur_task,
                    epoch + 1,
                    epochs,
    ...
    def eval_cur_task(self, cur_task, data_manager):
        self._cur_task = cur_task
        self._known_classes = self.inc * cur_task
        self._total_classes = self._known_classes + data_manager.get_task_size(self._cur_task) #data_manager._increments = [3, 3, 3, 3, 3]
        
        self._network.last_cls = data_manager.get_task_size(-1)
        self._network.nb_tasks = data_manager.nb_tasks
        ckpt_path = os.path.join(self.args["ckpt_path"],"{}_{}_{}_{}_woupdate.pkl".format(self.args["dataset"],self.args["seed"],self.args["use_old_data"],self._cur_task))
        self._network.load_state_dict(torch.load(ckpt_path)["model_state_dict"],strict=False)
        
        self._network.fc = self._network.generate_fc((cur_task+1)*self._network.out_dim, self._total_classes).to(self._network._device)
        self._network.fc.load_state_dict(torch.load(ckpt_path)["fc_weight_data"],strict=False)
        self._network._cur_task = cur_task
