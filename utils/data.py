...
class ModelNet(iData):
    ...
    def download_data(self, config = None):
        self.root = '/data/dataset/aizixiang/ModelNet/modelnet40/modelnet40_normal_resampled/' #config.DATA_PATH
        self.save_root = '/data/dataset/zhangyanqin/ModelNet/modelnet40_normal_resampled/'
        ...
        if self.uniform:
            self.train_save_path = os.path.join(self.save_root, 'modelnet%d_%s_%dpts_fps.dat' % (self.num_category, 'train', self.npoints))
            self.test_save_path = os.path.join(self.save_root, 'modelnet%d_%s_%dpts_fps.dat' % (self.num_category, 'test', self.npoints))
        else:
            self.train_save_path = os.path.join(self.save_root,'modelnet%d_%s_%dpts.dat' % (self.num_category, 'train', self.npoints))
            self.test_save_path = os.path.join(self.save_root, 'modelnet%d_%s_%dpts.dat' % (self.num_category, 'test', self.npoints))
...
class ShapeNet(iData):
    ...
    def download_data(self, config = None):
        self.root = '/data/dataset/aizixiang/ShapeNet55/' #config.DATA_PATH
        self.save_root = '/data/dataset/zhangyanqin/ShapeNet55/'
        self.uniform = True
        self.process_data = False
        self.npoints = 1024  # config.N_POINTS
        self.use_normals = False  # config.USE_NORMALS
        ...
        shape_ids = {}
        shape_ids['train'] = [line.rstrip() for line in open(os.path.join(self.save_root, 'train.txt'))]
        shape_ids['test'] = [line.rstrip() for line in open(os.path.join(self.save_root, 'test.txt'))]
        ...
        if self.uniform:
            self.train_save_path = os.path.join(self.save_root, 'shapenet%d_%s_%dpts_fps.dat' % (self.num_category, 'train', self.npoints))
            self.test_save_path = os.path.join(self.save_root, 'shapenet%d_%s_%dpts_fps.dat' % (self.num_category, 'test', self.npoints))
        else:
            self.train_save_path = os.path.join(self.save_root,'shapenet%d_%s_%dpts.dat' % (self.num_category, 'train', self.npoints))
            self.test_save_path = os.path.join(self.save_root, 'shapene%d_%s_%dpts.dat' % (self.num_category, 'test', self.npoints))
...
class ScanObject(iData):
    ...
    def download_data(self, config = None):
        self.root = '/data/dataset/aizixiang/ScanObjectNN/scanobjectnn/main_split_nobg/'
        self.save_root = '/data/dataset/zhangyanqin/scanobjectnn/main_split_nobg'
        self.npoints = 1024  # config.N_POINTS
        self.uniform = True
        self.process_data = False
        ...
        if self.uniform:
            self.train_save_path = os.path.join(self.save_root, 'scanobject%d_%s_%dpts_fps.dat' % (self.num_category, 'train', self.npoints))
            self.test_save_path = os.path.join(self.save_root, 'scanobject%d_%s_%dpts_fps.dat' % (self.num_category, 'test', self.npoints))
        else:
            self.train_save_path = os.path.join(self.save_root,'scanobject%d_%s_%dpts.dat' % (self.num_category, 'train', self.npoints))
            self.test_save_path = os.path.join(self.save_root, 'scanobject%d_%s_%dpts.dat' % (self.num_category, 'test', self.npoints))
        ...
        if self.process_data:
            ...
            train_data_tmpt = np.array(train_file['data']).astype(np.float32) #shape is (11426, 2048, 3)
            train_targets_tmpt = np.array(train_file['label']).astype(int) #shape is (11426,)
            test_data_tmpt = np.array(test_file['data']).astype(np.float32) #shape is (2887, 2048, 3)
            test_targets_tmpt = np.array(test_file['label']).astype(int) #hape is (2887,)
