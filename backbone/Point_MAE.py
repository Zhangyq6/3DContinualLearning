...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, pos, global_feature=None, token_position=None):
        for idx, block in enumerate(self.blocks):
            if idx < 6 and global_feature is not None:
...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, eval=False,label=None, **kwargs):

        neighborhood, center = self.group_divider(pts)

        if eval:
            # for linear svm
            x_vis, mask = self.MAE_encoder(neighborhood, center, eval=True)

            return x_vis.max(dim=1)[0]
